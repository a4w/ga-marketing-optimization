...
public class NonUniformMutation implements IMutationAlgorithm<ChannelAllocationChromosome> {
    ...
    public ChannelAllocationChromosome mutate(ChannelAllocationChromosome chromosome,
        ...
        for (int i = 0; i < genes.size(); i++) {
            if (RandomGenerator.randFloat(0, 1) > config.getMutationProbability()) // skip mutation of current gene if
            ...
            assert (genes.get(i) >= metaData.getChannelLowerLimit(i)
                    && genes.get(i) <= metaData.getChannelUpperLimit(i)); // assertion that gene is still within limits
                                                                          // after mutation
    ...
    /**
     * b ( (1 - t/T) ^ ) delta = range * (1 - r ^ ) r: a random float within [0:1]
     * t: the current generation T: the max. number of generations b: a float
     * parameter that controls the degree of non-uniformity within [0.5, 5]
     */
